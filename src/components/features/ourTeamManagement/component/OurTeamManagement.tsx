...
import { useOurTeam, useDeleteTeamMember } from "../hook/useOurTeamManagement";
...
export default function OurTeamManagement() {
  const [page, setPage] = useState(1);
  const [limit] = useState(10);
  const { data, isLoading } = useOurTeam(page, limit);
  ...
  const { mutate: deleteMemberMutation, isPending: isDeleting } =
    useDeleteTeamMember();
  ...
  const handleDeleteConfirm = () => {
    ...
    deleteMemberMutation(memberToDelete, {
      onSuccess: () => {
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <HeaderTitle title="Team Management" subtitle="Dashboard > Our Team" />
        <Button
            ...
            <tbody className="divide-y divide-gray-50">
              {isLoading ? (
                Array.from({ length: 5 }).map((_, idx) => (
                  <tr key={`team-table-skeleton-${idx}`}>
                  ...
                  {data?.data.length === 0 ? (
                    ...
                    data?.data.map((item) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 text-sm text-gray-600 font-semibold uppercase tracking-wider text-[10px]">
